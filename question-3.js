...
const getVacancyMessage = (rental, bookings = []) => {
  ...
  const vacanciesLeftNumber = rental.totalNumberAvailable - bookingsNumber;
  // Return the right message using the MESSAGE object
  if (vacanciesLeftNumber === 1) {
    return MESSAGE.ONLY1LEFT;
  } else if (vacanciesLeftNumber === 0) {
    return MESSAGE.UNAVAILABLE;
  } else if (vacanciesLeftNumber >= 2 && vacanciesLeftNumber <= 3) {
    return MESSAGE.ONLYXLEFT(vacanciesLeftNumber);
  } else {
